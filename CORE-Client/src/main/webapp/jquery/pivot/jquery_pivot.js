...
var methods = {
  ...
  process : function(options){
    ...
    if (options.skipBuildContainers === undefined || options.skipBuildContainers === false) self.build_containers(options);
  ...
  build_containers : function(options){
	var filterFieldsHeader = 'Filter Fields';
	var rowLabelFieldsHeader = 'Row Label Fields';
	var columnLabelFieldsHeader = 'Column Label Fields';
	var summaryFieldsHeader = 'Summary Fields';
	
	if(options.menuContainer.filterFieldsHeader !== undefined && options.menuContainer.filterFieldsHeader !== '' ){
		filterFieldsHeader = options.menuContainer.filterFieldsHeader;
	}
	if(options.menuContainer.rowLabelFieldsHeader !== undefined && options.menuContainer.rowLabelFieldsHeader !== '' ){
		rowLabelFieldsHeader = options.menuContainer.rowLabelFieldsHeader;
	}
	if(options.menuContainer.columnLabelFieldsHeader !== undefined && options.menuContainer.columnLabelFieldsHeader !== '' ){
		columnLabelFieldsHeader = options.menuContainer.columnLabelFieldsHeader;
	}
	if(options.menuContainer.summaryFieldsHeader !== undefined && options.menuContainer.summaryFieldsHeader !== '' ){
		summaryFieldsHeader = options.menuContainer.summaryFieldsHeader;
	}

	
    var containers = '<div class="pivot_header_fields">' +
                     '  <div class="pivot_field">' +
                     '  <span class="pivot_header2">' +filterFieldsHeader+ '</span>' +
                     '   <div id="filter-list"></div>' +
                     '  </div>' +
                     '  <div class="pivot_field">' +
                     '  <span class="pivot_header2">' +rowLabelFieldsHeader+ '</span>' +
                     '   <div id="row-label-fields"></div>' +
                     '  </div>' +
                     '  <div class="pivot_field">' +
                     '  <span class="pivot_header2">' +columnLabelFieldsHeader+ '</span>' +
                     '   <div id="column-label-fields"></div>' +
                     '  </div>' +
                     '  <div class="pivot_field">' +
                     '  <span class="pivot_header2">' +summaryFieldsHeader+ '</span>' +
  ...
  build_filter_list : function(){
    ...
    $.each(pivot.fields().filterable, function(index, field){
      ...
      select += '<option value="' +field.name+ '">' + field.label + '</option>';
    })
    select += '</select>'
  ...
  build_filter_field : function(fieldName, selectedValue) {
    ...
    remove_filter = '<span class="remove-filter-field" style="cursor:pointer;">(X)</span></label>';
    $('#filter-list').append('<div><hr/><label>' + field.label + remove_filter + snip + '</div>');
  ...
  update_results : function(){
	...
	snip += '<table id="' +tableId+ '" class="table table-striped table-condensed ' +tableClass+ ' " width="100%"><thead>';
    ...
    if (config.columnLabels.length > 0 && config.summaries.length > 1) {
      ...
      $.each(config.summaries, function(index, fieldName){
        summarySnip += '<th>' + pivot.fields().get(fieldName).label + '</th>';
        //summarySnip += '<th>' + fieldName + '</th>';
      })

      snip += '<tr>';
	  snip += '<th class="header-rowspan" rowspan="2"></th>';
      $.each(columns, function(index, column){
        switch (column.type){
          case 'row':
            snip += '<th class="header-rowspan" rowspan="2">'+ column.label + '</th>';
            break;
          case 'column':
            snip += '<th class ="header-colspan" colspan="' + column.width + '">' + column.label + '</th>';
    ...
    $.each(results,function(index, row){
      ...
      $.each(columns, function(index, column){
        if (column.type !== 'column'){
			snip += '<td align="' +column.field.align+ '">' + row[column.fieldName] + '</td>';
        }else {
          $.each(config.summaries, function(index, fieldName){

            if (row[column.fieldName] !== undefined){
              snip += '<td align="' + pivot.fields().get(fieldName).align + '">' + row[column.fieldName][fieldName] + '</td>';
            }else
              snip += '<td>&nbsp;</td>';
          });
        }
