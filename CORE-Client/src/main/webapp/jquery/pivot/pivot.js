...
function processHeaderRow(row){
  ...
  */
  function appendField(field){
    ...
    if (field.align        		=== undefined) field.align				= 'left';
    if (field.label        		=== undefined) field.label				= field.name;
...
function pivotData(type) {
  ...
  */
    ...
    while (++i < m) {
      ...
      for (var key in displayFields.rowLabels) {
        if (displayFields.rowLabels.hasOwnProperty(key)) {
          if (i === 0) resultsColumns.push({fieldName: key, width: 1, type: 'row', label : fields[key].label, field: fields[key]});
  ...
  function populateSummaryColumnsResults(){
    for (var key in displayFields.summaries){
		if (displayFields.summaries.hasOwnProperty(key)){
			...
			resultsColumns.push({fieldName: key, width: 1, type: 'summary', label : fields[key].label, field: fields[key]});
		}
    }
  ...
  function populateColumnLabelColumnsResults(key, columnLabels){
    ...
    while (++i < m){
      resultsColumns.push({fieldName: keys[i], width: w, type: 'column', label : keys[i], field: fields[key] })
    };
