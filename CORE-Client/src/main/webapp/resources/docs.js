...
DocPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
        ...
        Ext.apply(this,{
            tbar: ['->',{
				...
				text: 'Check',
				handler: this.checkRow.createDelegate(this,['form71',true]),
				scope: this,
                iconCls: 'icon-fav'
            }, '-', {
				text: 'Serialize',
				handler: function(){
					Ext.Msg.Alert(Ext.get('form7Form').serialize());
                },
				scope: this,
                iconCls: 'icon-fav'    
            }]
	...
	//ham dc goi trong button directLink
    directLink : function(){
	...
	checkRow : function (rowid,check){
		var recentRow = Ext.get(''+rowid);
		if(check){
			recentRow.setStyle('backgroundColor','#FFFF99');

		} else {
			recentRow.setStyle('backgroundColor','#FFFFFF');
		}
	}
});
...
//extend tab tu mainPanel sang DOcPanel
Ext.extend(MainPanel, Ext.TabPanel, {
	...
	//bat su kien onClick tren cay thu muc MainPanel
	...
	//Ham mo 1 tab: tab moi hoac tab dang dc mo truoc do.
    loadClass : function(href, cls, member){
        var id = 'docs-' + cls;

		 Ext.Msg.alert('href= ' + href + '; cls = ' +cls + '; member= ' + member + '; id=' + id);
        ...
        if(tab){ //neu tab da dc mo truoc do
            ...
            var autoLoad = {url: href}; //set url cho tab
